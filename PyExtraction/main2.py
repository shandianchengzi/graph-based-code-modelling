...
class NodeVisitor(object):
        ...
        def visit_Astimg(self, node):#节点可视化遍历
            # 获取节点类型名称组成访问器方法名（子类Interpreter中方法的名称）
            method_name = 'visit_Astimg_' + type(node).__name__
            # 获取访问器对象，找不到访问器时获取“generic_visit”
            visitor = getattr(self, method_name, self.generic_visit)
            return visitor(node)

        def generic_visit_Astimg(self, node):
            raise Exception('No visit_Astimg_{} method'.format(type(node).__name__))
...
class Interpreter(NodeVisitor):
    ...
    #  以下为AST各节点的可视化遍历方法-------------------
    def visit_Astimg_Program(self, node):
        blockdata=self.visit_Astimg(node.block)
        Programdata={"children":blockdata,"name": "Program"}
        data=[Programdata]
        return data

    def visit_Astimg_Block(self, node):
        decldata=[]
        for declaration in node.declarations:
            decldata.append(self.visit_Astimg(declaration))
        compdata=self.visit_Astimg(node.compound_statement)
        declandcompdata=decldata+compdata  #Block节点由变量声明和符合语句组成
        Blockdata={"children":declandcompdata,"name": "Block"}
        data = [Blockdata]
        return data

    def visit_Astimg_VarDecl(self, node):
        vardata=self.visit_Astimg_Variable(node.var_node)
        Typedata=self.visit_Astimg_Type(node.type_node)
        VarDecldata={"children":vardata+Typedata,"name": "VarDecl"}
        return VarDecldata

    def visit_Astimg_Variable(self, node):  # 访问变量节点
        var_name = node.name  # 获取变量名称
        vardata={"name": "Var\n"+var_name}
        data = [vardata]
        return data

    def visit_Astimg_Type(self, node):  # 添加访问类型的方法
        Typedata={"name": "Type\n"+node.name}
        data=[Typedata]
        return data

    def visit_Astimg_Compound(self, node):  # 访问复合语句节点
        comchild=[]
        for child in node.children:  # 遍历复合语句节点的子节点
            comchild.append(self.visit_Astimg(child))  # 访问子节点
        Compounddata={"children":comchild,"name": "Compound"}
        data=[Compounddata]
        return data

    def visit_Astimg_Assign(self, node):  # 访问赋值语句节点
        vardata={"name":"Var\n"+node.left.name}
        rightdata= self.visit_Astimg(node.right)
        Assignchild = [vardata]+rightdata
        Assigndata={"children":Assignchild, "name": "Assign"}
        return Assigndata

    def visit_Astimg_BinOp(self, node):  # 访问二元运算符类型节点的方法
        leftdata=self.visit_Astimg(node.left)
        rightdata=self.visit_Astimg(node.right)
        BinOpdata={"children":leftdata+rightdata,"name": "BinOp\n"+node.op.type}
        data = [BinOpdata]
        return data

    def visit_Astimg_Num(self, node):  # 访问数字类型节点的方法
        Numdata = {"name": "Num\n"+str(node.value)}
        data = [Numdata]
        return data

    def visit_Astimg_UnaryOp(self, node):#一元操作符
        op = node.op.type
        exprdata=self.visit_Astimg(node.expr)
        UnaryOpdata={"children":exprdata,"name": "UnaryOp:"+op}
        data = [UnaryOpdata]
        return data

    def visit_Astimg_NoOp(self, node):  # 访问空语句节点
        NoOpdata={"name": "NoOp"}
        data = [NoOpdata]
        return NoOpdata
    #  以上为AST各节点的可视化遍历方法-------------------

    def drawAST(self,tree):
        data = self.visit_Astimg(tree)  # 返回语法树可视化的遍历结果
        print('AST_Tree:', data)        # 显示AST树数据
        Asttree = (
            Tree()
                .add("", data, initial_tree_depth=-1, orient="TB")
                .set_global_opts(title_opts=opts.TitleOpts(title="AST树结构"))
                )
        Asttree.render(r'AstTree.html')

    def interpret(self):  # 执行解释的方法
        tree = self.parser.parse()  # 获取语法分析器分析后的树对象
        self.visit(tree)  # AST语法树遍历(解释)
        self.drawAST(tree) #绘制AST语法树图形
